...
To account for specific satellite channel conditions, we compute $h$ using Fontan's statistical channel~\cite{966585} model. 
...
Then, the propagation channel at a given time is described by the state probability matrix, which contains the probabilities of being in a particular state
...
In each state, the channel is modeled using Loo distribution~\cite{1623307}. 
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
where $I_0(\cdot)$ is the modified Bessel function of zeroth order.
The following dependencies exist between $\mu$, $d_0$ and $b_0$ and $\alpha$, $\psi$ and \MP:
\begin{align}
...
We simulate the Fontan's model as described in \cite{DBLP:journals/ijscn/Perez-FontanMMPMMR08}:
