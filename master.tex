...
Although different communication technologies, such as VLC~\cite{nakajima2012} and laser~\cite{welle2018}, have been considered for small satellites, RF-based approaches remain predominant. Therefore, we focus on channel models for satellite-to-ground RF-based communication.   
Loo's model~\cite{1623307} combines log-normal and Rayleigh distributions to model the LOS and multipath components respectively. Corazza-Vatalaro's model~\cite{Corazza1994ASM}, in contrast, uses the product of Rician and log-normal distributions to describe the multipath component. 
Various extensions for Loo's and Corazza-Vatalaro's models have been proposed~\cite{596315, 661055}.
All these models use a single distribution to describe all propagation conditions and a typically refered to as static. 
To model signal changes over time, dynamic models, which are mostly based on Markov chains, can be used.
Fontan's model~\cite{966585} considers three states, which correspond to different degrees of shadowing. Other examples for dynamic models include Scalise’s model~\cite{4151152}, Nikolaidis’ model~\cite{7779114} and Salamanca’s model~\cite{8693582}. A more detailed overview is provided in \cite{9079470}.
For our evaluation we chose Fontan's model. This model was tested for a broad range of frequencies in different environments and considers different elevation angles.   
